<?php foreach ($playlist_name as $playlistitem):?>
    <?php $playlistName = $playlistitem['Playlist']['name'];?>
<?php endforeach?>
<h1><?php echo $playlistName;?></h1>

<button class="add-btn-form"><a href=<?php echo BASE_PATH . DS . 'playlist' . DS . 'addsong' . DS . $playlist_id . DS ?>>Add song</a></button>
<br>
<?php $number = 0?>
<?php
    $song_name = [];
...
<?php for($i = 0; $i < sizeof($song_id); $i++):?>
        ...
        <li class="item">
            ...
            <button class="btn-delete"><a href=<?php echo BASE_PATH . DS . 'playlist' . DS . 'deletesong' . DS . $playlist_id . DS . $song_id[$i]?>>Delete</a></button>
		</li>
<?php endfor?>
